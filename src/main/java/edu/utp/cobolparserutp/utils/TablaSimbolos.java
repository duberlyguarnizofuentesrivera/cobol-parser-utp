...
public class TablaSimbolos {
  ...
  public void updateLastKey(String _type, String _value, int _numOfChar) {
    ...
    if (getTablaSimbolos().get(lastKeyEntered) == null) {
      semantic_errors(new SemanticError(lastKeyEntered,
          "No declarada"));
    } else {
  ...
  public String getTypeOfLastKeyEnteredAndNumOfCharSeparatedByCommas(
    ...
    if (getLastKeyEntered() != null) {
      ...
      // AlfanumŽricas: se indican con un PIC X(nœmero de
      // caracteres). Podr‡n tener como valores nœmeros y letras.
  ...
  public boolean checkKey(String _key) {
    if (getTablaSimbolos().get(_key) == null) {
      semantic_errors(new SemanticError(_key, "No declarada"));
      return false;
    }
    if (clause == "DISPLAY") {
   ...
   * @param error
   */
  public void semantic_errors(SemanticError error) {
    FileWriter fw = null;
    PrintWriter pw = null;
